...
def blend_interp_candidates(word, dic):
    ...
    #    common.
    #
    other_words = [x for x in dic if \
                   # 1.
                   x.isalpha() \
                   # 2.
                   and not x == word \
                   # 6.
                   and not x == word_stem and not stemmer.stem(x) == word \
                   and not stemmer.stem(x) == word_stem \
                   # 7.
    ...
    for w1 in w1s:
        ...
        for w2 in w2s:
            ...
            if len(lcp_w1_word) + len(lcs_w2_word) >= len(word) \
                    ...
                    and not w2.startswith(w1) and not w1.endswith(w2):
                print(word)
                return word
    return "-1"
...
def all_blend_interp_candidates(clean_candidate, dic, blends):
    # Return candidate blend interpretations for all out-of-vocabulary
    # (according to lexicon) words in context_tokens
    count = 0
    all_bics = []
    for o in clean_candidate:
        bics = blend_interp_candidates(o, dic)
        if bics != "-1":
            all_bics.append(bics)
    for al in all_bics:
        if al in blends:
            count += 1
    return count / len(all_bics)

stemmer = nltk.stem.PorterStemmer()
candidate, dic, blends = process_date()
print(all_blend_interp_candidates(candidate, dic, blends))
